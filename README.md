...
| | 是什麼 | 怎麼決定 |
|---|---|---|
| **承辦股** | 案件的承辦單位，需要支援 | 已確定，抽籤時與案號一併輸入 |
| **支援股** | 支援該案的單位，共 8 個股分屬 4 庭 | **抽籤決定**，籤筒輪分的就是這個 |

兩者是不相交的名單——承辦股不在支援股的籤筒內，不會被抽為自己案件的支援股。
設定檔中 `units` 是支援股，`requesters` 是承辦股，兩份名單都沒有數量上限。

**角色名稱可以改。** 抽籤機制與任務內容無關，本次支援任務結束後若要改做其他
輪分任務（例如值班股抽籤），只需在「組織管理」頁改用詞與兩份名單，
**不必更動已驗證過的核心演算法**——改程式意味著要重新驗證整套演算法。
...
籤筒裡放的是**支援股**的籤，抽出的是該案的支援股。

- 金訴、金重訴**各有獨立籤筒**，互不影響。
- 初始每個籤筒放入 8 支籤（每個支援股 1 支），自籤筒內**剩餘的籤**等機率抽出。
- 籤筒剩 1 支，或剩 2 支且同屬一庭時，**自動補入全部支援股的籤**。
- 重大案件可設定抵分。抵 M 件 = 本案算 1 件 + 另扣**抽中的支援股** M−1 支籤，
  **優先扣籤筒內現有的籤**，不足額記為欠籤，於下次補籤後優先扣除。
- 抵分與迴避**逐件設定**：可抵分的案件是少數，整批共用會讓不需抵分的案件被誤抵。
- 迴避股的籤留在籤筒內，僅該件不可被抽中。
- 支援股變動立即生效；新股可設定每輪多支籤以追分。
- 承辦股不參與抽籤，新增或停用承辦股不影響籤筒。
...
| 支援股 → 承辦股（`move-to-requester`） | **立即撤出**其籤並執行補籤檢查 |
| 承辦股 → 支援股（`move-to-drawee`） | **立即投入**其籤並執行補籤檢查 |
| 新增／停用承辦股 | 不影響籤筒（承辦股不參與抽籤） |
| 修改角色用詞（`set-term`） | 不影響籤筒，全站用語一併變更 |
